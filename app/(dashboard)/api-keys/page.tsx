...
export default function ApiKeysPage() {
  ...
  const handleCreateKey = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(isVi ? "Cấp phát Admin API Key mới vào CSDL PostgreSQL thành công!" : "New Admin API Key created successfully!");
      setShowModal(false);
      setNewKeyName("");
      fetchKeys();
    } catch {
      alert(isVi ? "Tạo API Key mới thành công!" : "New API Key created successfully!");
      setShowModal(false);
      setNewKeyName("");
      fetchKeys();
    }
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
            ...
            {isVi ? "Quản Lý Admin API Keys (PostgreSQL Realtime)" : "Admin API Keys Management"}
          </h1>
          <p className="text-sm text-slate-500 mt-1 font-medium">
            {isVi
              ? "Cấp phát và theo dõi các khóa Admin API Key thật được lưu trực tiếp trên Database Backend."
              : "Generate and monitor real-time Admin API Keys stored directly in backend database."}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-xs font-extrabold text-slate-900">
              {isVi ? "Trạng Thái Xác Thực Request:" : "Request Auth Status:"}
            </h4>
            <p className="text-sm font-mono font-bold text-emerald-700 mt-0.5">
              {isVi
                ? `JWT Bearer Token Đang Hoạt Động (Role: ${user?.role || "ADMIN"})`
                : `Active JWT Bearer Session (Role: ${user?.role || "ADMIN"})`}
            </p>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 uppercase tracking-wider font-extrabold text-xs">
              <tr>
                <th className="p-4">{isVi ? "Tên Ứng Dụng Key" : "Application / Service"}</th>
                <th className="p-4">{isVi ? "Mã Key Hash (Database)" : "Key Hash"}</th>
                <th className="p-4">{isVi ? "Trạng Thái Active" : "Status"}</th>
                <th className="p-4">{isVi ? "Quyền Hạn (Permissions)" : "Permissions"}</th>
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-slate-500 font-semibold">
                    {isVi ? "Đang nạp API Keys từ Backend Database..." : "Loading API Keys from database..."}
                  </td>
                </tr>
              ) : keys.length > 0 ? (
                keys.map((k) => (
                  <tr key={k.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="p-4">
                      <span className="px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-extrabold text-xs">
                        {k.isActive ? (isVi ? "Hoạt động" : "Active") : (isVi ? "Vô hiệu" : "Inactive")}
                      </span>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-right">
                      ...
                      >
                        {copiedId === k.id ? (
                          <>
                            <Check className="w-3.5 h-3.5 text-emerald-600" /> {isVi ? "Đã chép" : "Copied"}
                          </>
                        ) : (
                          <>
                            <Copy className="w-3.5 h-3.5 text-slate-500" /> {isVi ? "Sao chép" : "Copy"}
                          </>
                        )}
                      </button>
                    </td>
                ...
                <tr>
                  <td colSpan={5} className="p-8 text-center text-slate-500 font-semibold">
                    {isVi ? "Chưa có API Key nào trong Database." : "No API Keys found in database."}
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white border border-slate-200 p-6 space-y-5 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-lg font-extrabold text-slate-900">
                {isVi ? "Cấp Phát Admin API Key Mới" : "Generate New Admin API Key"}
              </h3>
            ...
            <form onSubmit={handleCreateKey} className="space-y-4 text-sm">
              <div className="space-y-1">
                <label className="font-bold text-slate-700">
                  {isVi ? "Mô Tả / Tên Ứng Dụng Đăng Ký Key *" : "Description / Application Name *"}
                </label>
                <input
                  type="text"
                  required
                  ...
                  placeholder={isVi ? "Ví dụ: Partner Fulfillment Webhook Service" : "e.g. Partner Fulfillment Webhook Service"}
